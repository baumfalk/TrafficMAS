...
public class DataModelXML implements DataModel {
	...
	@Override
	public ArrayList<Pair<Integer, Agent>> instantiateAgents(Random rng, ArrayList<Route> routes) {

		
		return instantiateAgents(rng,routes, simulationLength, agentSpawnProbability, agentProfileDistribution );
	}
	
	public static ArrayList<Pair<Integer, Agent>> instantiateAgents(Random rng, ArrayList<Route> routes, int simulationLength, double agentSpawnProbability, ArrayList<Pair<AgentProfileType, Double>> agentProfileDistribution) {
		ArrayList<Pair<Integer, Agent>> agentsAndTimes = new ArrayList<Pair<Integer,Agent>>(); 
		for (int i = 0; i < simulationLength; i++) {
			...
			if(coinFlip < agentSpawnProbability) {
				...
				agentsAndTimes.add(new Pair<Integer,Agent>(i,agent));
			}
		}
